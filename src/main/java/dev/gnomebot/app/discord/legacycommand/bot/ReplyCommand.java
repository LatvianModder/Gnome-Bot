...
public class ReplyCommand {
	@LegacyDiscordCommand(name = "reply", arguments = "<text...>", permissionLevel = AuthLevel.BOT)
	public static final CommandCallback COMMAND = (context, reader) -> {
		...
		if (context.interaction instanceof ComponentEventWrapper) {
			((ComponentEventWrapper) context.interaction).respond(text);
			...
			((ApplicationCommandEventWrapper) context.interaction).respond(text);
		} else {
			context.reply(text);
		}
	};
